...
def mls_classification(K_all, K_chk, y_chk, mls,
                       hdf=None, out_class=None, key=None):
    res = {}
    for k in mls if key is None else [key, ]:
        print(k)
        res[k] = classify(K_all, K_chk, y_chk, mls[k],
                          save=out_class % k if out_class else '', hdf=hdf)
    return res
